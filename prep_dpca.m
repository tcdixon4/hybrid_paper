function [X] = prep_dpca(unit_data, norm_method)
...
for unit = length(unit_data):-1:1
    ...
    for config = 3:-1:1
        for target = 6:-1:1
            ...
            % 43:68 - Move, (Move-200ms):(Move+300ms), 26 samples at 50Hz
            if isempty(l_hand.config(config).target(target).rest)
                l_hand_avg(unit,:,target,config) = nan(1,68);
            else
                l_hand_avg(unit,:,target,config) = [...
                    ...
                    mean(l_hand.config(config).target(target).move(:,1:26),1)]';
            end
            if isempty(r_hand.config(config).target(target).rest)
                r_hand_avg(unit,:,target,config) = nan(1,68);
            else
                r_hand_avg(unit,:,target,config) = [...
                    ...
                    mean(r_hand.config(config).target(target).move(:,1:26),1)]';                
            end
        end
    ...
    for config = 3:-1:1
        ...
        X.l_hand.config(config).move(unit,:,:) =...
            (l_hand_avg(unit,43:68,:,config) - mu)/norm_constant;
        ...
        X.r_hand.config(config).move(unit,:,:) =...
            (r_hand_avg(unit,43:68,:,config) - mu)/norm_constant;
    end
